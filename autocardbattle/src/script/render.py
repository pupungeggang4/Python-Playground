...
class Render():
    ...
    @staticmethod
    def render_field(surface, game):
        surface.blit(Font.neodgm_32.render(f'Turn : {game.battle.turn}', False, Color.black), UI.Battle.text_turn_num)
        if game.battle.turn_who == 0:
            surface.blit(Font.neodgm_32.render('Your turn', False, Color.black), UI.Battle.text_whose_turn)
        else:
            surface.blit(Font.neodgm_32.render('Enemy turn', False, Color.black), UI.Battle.text_whose_turn)
        for i in range(10):
            if game.battle.field[i] != None:
                game.battle.field[i].render(game.surface, game, UI.Battle.field[i])
            pygame.draw.rect(surface, Color.black, UI.Battle.field[i], 2)

        surface.blit(Image.button['play'], UI.Battle.button_play)
        surface.blit(Image.button['pause'], UI.Battle.button_pause)
        surface.blit(Image.button['menu'], UI.Battle.button_menu)
        pygame.draw.rect(surface, Color.black, UI.Battle.button_proceed, 2)
        surface.blit(Font.neodgm_32.render('Proceed', False, Color.black), UI.Battle.text_proceed)

    @staticmethod
    def render_crystal(surface, game):
        pygame.draw.rect(surface, Color.black, UI.Battle.player_crystal_box, 2)
        for i in range(len(game.battle.player.crystal_hand)):
            ...
            game.battle.player.crystal_hand[i].render(surface, game, pos)
        pygame.draw.rect(surface, Color.black, UI.Battle.enemy_crystal_box, 2)
        for i in range(len(game.battle.enemy.crystal_hand)):
            ...
            game.battle.enemy.crystal_hand[i].render(surface, game, pos)

    @staticmethod
    def render_reward_window(surface, game):
        pygame.draw.rect(surface, Color.white, UI.Window.rect)
        pygame.draw.rect(surface, Color.black, UI.Window.rect, 2)
        
        surface.blit(Font.neodgm_32.render('Select Reward', False, Color.black), UI.Window.text_title)
