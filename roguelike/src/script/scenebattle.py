...
def key_down(game, key):
    if game.menu == False:
        ...
        if game.state == 'adventure_start':
            handle_key_adventure_start(game, key)
        elif game.state == '':
            handle_key_battle(game, key)
...
def handle_key_adventure_start(game, key):
    if key == pygame.K_a:
...
def handle_key_battle(game, key):
    ...
    if button == 1:
        if game.menu == False:
            if point_inside_rect_ui(pos, UI.Battle.button_menu):
                game.menu = True
            if game.state == 'adventure_start':
                handle_mouse_up_adventure_start(game, pos, button)
            elif game.state == '':
...
def handle_mouse_up_adventure_start(game, pos, button):
    for i in range(3):
